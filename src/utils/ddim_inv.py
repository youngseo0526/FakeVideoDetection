...
class DDIMInversion(BasePipeline):
    ...
    def __call__(
        ...
        x0 = (x0 - 0.5) * 2. # torch.Size([1, 3, 512, 512])
        with torch.no_grad():
            x0_enc = self.vae.encode(x0).latent_dist.sample().to(device, torch_dtype) # torch.Size([1, 4, 64, 64])
        ...
        with torch.no_grad():
            x0_dec = self.decode_latents(x0_enc.detach()) # (1, 512, 512, 3)
        image_x0_dec = self.numpy_to_pil(x0_dec)

        with torch.no_grad():
            prompt_embeds = self._encode_prompt(prompt, device, num_images_per_prompt, do_classifier_free_guidance, negative_prompt).to(device) # torch.Size([1, 77, 768])
        ...
        with self.progress_bar(total=num_inversion_steps) as progress_bar:
            for i, t in enumerate(timesteps.flip(0)[1:-1]):
                ...
                latent_model_input = torch.cat([latents] * 2) if do_classifier_free_guidance else latents # torch.Size([1, 4, 64, 64])
                latent_model_input = self.scheduler.scale_model_input(latent_model_input, t)

                # predict the noise residual
                with torch.no_grad():
                    #noise_pred = self.unet(latent_model_input,t,encoder_hidden_states=prompt_embeds,cross_attention_kwargs=cross_attention_kwargs,).sample  # torch.Size([1, 4, 64, 64])
                    noise_pred = self.unet(latent_model_input,t,encoder_hidden_states=prompt_embeds).sample  # torch.Size([1, 4, 64, 64])
                # perform guidance
