...
class TrainDataset(Dataset):
    def __init__(self, data_dir, transform=None):
        ...
        fake_dir = os.path.join("/data/yskim/easy-diffusion-generation/generated_images/fake_inversion_prompts/sd-15_det-seed") 
        real_dir = os.path.join("/data/yskim/LAION-Aesthetics-V2-6.5plus/data") 
        
        for file in glob(os.path.join(fake_dir, "*.png")):
            self.data_files.append(file)
            self.labels.append(0) # label: 0

        for file in glob(os.path.join(real_dir, "*.png")):
            self.data_files.append(file)
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(data_path).convert("RGB")  # Load image and convert to RGB
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_save_path', type=str, default='/data/yskim/pix2pix-zero/fake_sdv1-real_laion_imgonly.pth')
    ...
    transform = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[0.5, 0.5, 0.5], std=[0.5, 0.5, 0.5])  # Adjust for RGB
    ])
